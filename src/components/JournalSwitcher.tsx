...
export function JournalSwitcher() {
  ...
  const iconBtn =
    'flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-muted transition-colors hover:bg-white/[0.08] hover:text-ink'
        ...
        <>
          ...
          <div className="panel absolute z-30 mt-2 w-full min-w-[15rem] origin-top animate-zoom-in overflow-hidden p-1 text-right">
            ...
            {journals.map((j) => {
              ...
              return (
                <div key={j.id} className="group flex items-center gap-1 rounded-lg hover:bg-white/[0.05]">
                  <button
                    onClick={() => {
                      setActive(j.id)
                      setOpen(false)
