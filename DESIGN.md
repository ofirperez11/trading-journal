...
- **Hebrew (RTL) fallback: `Heebo`** (Google Fonts). This is an intentional brand addition — this product's UI is Hebrew, and Inter has no Hebrew glyphs. On Apple devices `system-ui` already resolves to SF Pro (which includes Hebrew); Heebo covers non-Apple browsers with a clean, SF-adjacent Hebrew sans.
